...
function start() {
  const tbody = document.getElementById("series-tbody") as HTMLTableSectionElement | null;
  const avgSpan = document.getElementById("avg-value") as HTMLSpanElement | null;
  const detail = document.getElementById("series-detail") as HTMLDivElement | null;
  ...
  if (series.length) renderDetail(series[0], detail);
}

function renderDetail(s: {
  name: string; channel: string; seasons: number;
